...
activeShot = () => {
    shotW = 1 * Math.sign(angle);
    shotH = Math.abs(((h - dragY)/(dragX - 0.5 * w))) * Math.abs(shotW);
    document.querySelector(".lazer").style.left = `${0.5 * w}px`;
...
shotAnimation = () => {
    ...
    document.querySelector(".lazer").style.bottom = `${parseFloat(document.querySelector(".lazer").style.bottom) + shotH}px`;
    document.querySelector(".lazer").style.left = `${parseFloat(document.querySelector(".lazer").style.left) + shotW}px`;
    ...
    if ((asteroidRect.y <= shotRect.y && shotRect.y <= asteroidRect.bottom && asteroidRect.x <= shotRect.x && shotRect.x <= asteroidRect.right)) {
        ...
        win();
    } else if ((parseFloat(document.querySelector(".lazer").style.left) > w) || (parseFloat(document.querySelector(".lazer").style.left) < 0)) {
            // user surpassed height
        if ((parseFloat(document.querySelector(".lazer").style.bottom) > h)) {
            shotW = 0;
            angle = 0;
            clearInterval(shotFunc);
            document.querySelector(".lazer").style.cssText = "transform: rotate(0deg); left: 50%; bottom: 0vh;";
